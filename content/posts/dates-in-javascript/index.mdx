...
date: '2019-03-12'
description: 'Working with dates in JS can be a complicated task. Personally, it always has been my weak part, so I decided to write a post about dates...'
image: ./code.png
tags: ['javascript']
---
...
<Alert type="info">
  <p>
    Using only the <code>setFullYear()</code> it's possible to change year, month, and day together.
    For example: <code>date.setFullYear(2019, 3, 20)</code>
  </p>
</Alert>
...
<Alert type="info">
  Since <code>Date</code> is an object, it's not possible to simply compare two different dates
  using <code>===</code> or <code>==</code>. JS will compare dates by their reference and not
  values.
</Alert>
